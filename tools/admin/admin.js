...
const elements = {
  ...
  contentHtml: document.querySelector("#contentHtml"),
  ...
  addEmbedButton: document.querySelector("#add-embed-button"),
...
const projectForForm = (project = createBlankProject()) => ({
  ...
  media: mediaItemsForProject(project).length
    ...
    : [{ type: "image", provider: "", src: "", alt: "", caption: "", credit: "", thumbnail: "", width: "", height: "" }],
...
const getImageRowValues = (row) => ({
  ...
  credit: row.querySelector("[data-field='credit']").value.trim(),
...
const createImageRow = (image = {}) => {
  ...
  row.querySelector("[data-field='credit']").value = image.credit || "";
...
const buildProjectFromForm = () => {
  ...
  const project = {
    ...
    ...(elements.date.value.trim() ? { date: elements.date.value.trim() } : {}),
    projectType: elements.projectType.value.trim(),
    role: elements.role.value.trim(),
    ...(elements.locationVenue.value.trim() ? { locationVenue: elements.locationVenue.value.trim() } : {}),
    ...(elements.collaborators.value.trim() ? { collaborators: elements.collaborators.value.trim() } : {}),
    ...
    links,
    media,
    ...(elements.thumbnail.value.trim() ? { thumbnail: elements.thumbnail.value.trim() } : {})
  };
...
const populateForm = (project, index = null) => {
  ...
  elements.contentHtml.value = safe.contentHtml;
...
const bindEvents = () => {
  ...
  elements.addEmbedButton.addEventListener("click", () => {
    const row = createImageRow({ type: "embed", provider: "url" });
    elements.imageList.append(row);
    setActiveImageRow(row.dataset.rowId);
    refreshPreview();
  });
